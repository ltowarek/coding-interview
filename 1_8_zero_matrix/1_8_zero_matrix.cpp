...
void Print(const Matrix<T, M, N> &m) {
...
TEST(zero_matrix_test, NoZeros_2x2) {
  const auto input = Matrix<int, 2, 2>{{{1, 2},
                                        {3, 4}}};
  EXPECT_EQ(ZeroMatrix(input), input);
}

TEST(zero_matrix_test, Zeros_2x2_0) {
  const auto input = Matrix<int, 2, 2>{{{0, 2},
                                        {3, 4}}};
  const auto expected = Matrix<int, 2, 2>{{{0, 0},
                                           {0, 4}}};
  EXPECT_EQ(ZeroMatrix(input), expected);
}

TEST(zero_matrix_test, Zeros_2x2_1) {
  const auto input = Matrix<int, 2, 2>{{{1, 0},
                                        {3, 4}}};
  const auto expected = Matrix<int, 2, 2>{{{0, 0},
                                           {3, 0}}};
  EXPECT_EQ(ZeroMatrix(input), expected);
...
TEST(zero_matrix_test, Zeros_2x2_2) {
  const auto input = Matrix<int, 2, 2>{{{1, 2},
                                        {0, 4}}};
  const auto expected = Matrix<int, 2, 2>{{{0, 2},
                                           {0, 0}}};
  EXPECT_EQ(ZeroMatrix(input), expected);
}
...
TEST(zero_matrix_test, Zeros_2x2_3) {
  const auto input = Matrix<int, 2, 2>{{{1, 2},
                                        {3, 0}}};
  const auto expected = Matrix<int, 2, 2>{{{1, 0},
                                           {0, 0}}};
  EXPECT_EQ(ZeroMatrix(input), expected);
}

TEST(zero_matrix_test, NoZeros_3x3) {
  const auto input = Matrix<int, 3, 3>{{{1, 2, 3},
                                        {4, 5, 6},
                                        {7, 8, 9}}};
  EXPECT_EQ(ZeroMatrix(input), input);
}
...
TEST(zero_matrix_test, Zeros_3x3_4) {
  const auto input = Matrix<int, 3, 3>{{{1, 2, 3},
                                        {4, 0, 6},
                                        {7, 8, 9}}};
  const auto expected = Matrix<int, 3, 3>{{{1, 0, 3},
                                           {0, 0, 0},
                                           {7, 0, 9}}};
  EXPECT_EQ(ZeroMatrix(input), expected);
}
